...
namespace cds {
    ...
    // copy rebuilds the rank/select over the copied bits. move is memberwise.
    //
    // The select shares the rank's index when it layers on it (select9 over
    ...
    public:
        ...
        // rank1(i) and the bit at i together, fused into one word load when the
        // rank supports it (rank9). Precondition: i < size.
        ...
        // Non-owning view over this dict's own bits and index. Valid only while
        // this bit_dict stays alive.
        [[nodiscard]] auto as_view() const noexcept {
            return bit_dict_view(m_bits.as_const_view(), m_rank.as_view(), m_select.as_view());
        }

    private:
        // Build the select from the shared rank if it layers on it, else
        // standalone from the bits.
    ...
    // Non-owning counterpart of bit_dict.
    template <typename BitVecView, typename RankView, typename SelectView> class bit_dict_view {
    public:
