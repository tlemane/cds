...
namespace cds::detail {
    ...
    struct packed_save_header {
        ...
        std::uint8_t width;
        std::uint8_t endian; // underlying value of pack_endian
        std::uint8_t mode;   // underlying value of pack_mode
        std::uint8_t reserved{0};
        std::uint64_t size; // element count, not word count
    };
    static_assert(io::mmap_aligned_header<packed_save_header>);

    inline constexpr std::uint32_t packed_save_magic = 0x64737064u;
