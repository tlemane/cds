...
namespace cds::broadword {
    ...
    // popcount_below<Endian>(w, i): number of set bits among the first
    // `i` bits of `w`, counted from the Endian side
    ...
    namespace detail {
        // Sean Eron Anderson (https://graphics.stanford.edu/~seander/bithacks.html#SelectPosFromMSBRank),
        //
        ...
        [[nodiscard]] constexpr std::size_t select_in_word_lsb_portable(std::uint64_t w,
            ...
            const std::uint64_t a = w - ((w >> 1) & (ones / 3));
            const std::uint64_t b = (a & (ones / 5)) + ((a >> 2) & (ones / 5));
            const std::uint64_t c = (b + (b >> 4)) & (ones / 0x11);
            const std::uint64_t d = (c + (c >> 8)) & (ones / 0x101);

            std::uint64_t kk = k;
            std::uint64_t t = (d >> 32) + (d >> 48);
            std::uint64_t s = 64;
            s -= ((t - kk) & 256) >> 3;
            kk -= (t & ((t - kk) >> 8));
            t = (d >> (s - 16)) & 0xff;
    ...
    // select_in_word<Endian>(w, r): position of the r-th set bit (r
    // 0-indexed), counted from the Endian side.
    template <pack_endian Endian>
        requires(Endian == pack_endian::lsb || Endian == pack_endian::msb)
    ...
    [[nodiscard]] constexpr std::size_t select_in_word_lsb_scan(Word w, std::size_t r) noexcept {
        ...
        return d; // unreachable given the precondition
    }
