...
namespace cds::detail {
    ...
    inline constexpr std::uint32_t wavelet_magic = 0x776D7831u; // "wmx1"

    template <typename Derived, typename Symbol> class wavelet_ops {
    public:
        using symbol_type = Symbol;

        // Symbol at position i. Precondition: i < size().
        ...
        // Occurrences of symbol c in [0, i). Precondition: i <= size().
        ...
        // Position of the r-th c (0-indexed). Precondition: r < rank(c, size()).
...
namespace cds {
    ...
    // wavelet_matrix_view: non-owning counterpart, over pre-built level views.
    template <typename Symbol = std::uint64_t, std::size_t Bits = 0,
