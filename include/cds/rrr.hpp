...
namespace cds::detail {
    ...
    // msb falls back to the bit loop. The read never runs past the source last
    ...
    inline constexpr std::uint32_t rrr_magic = io::cds_magic(io::format_id::rrr);
    ...
    [[nodiscard]] inline std::size_t rrr_decode_select(std::uint64_t nr, std::size_t len,
        ...
        const auto& t = rrr_binomial_table<BlockSize>();
        // High-to-low enumeration yields the (k-1), (k-2), ... one, so the
        // need-th one from the low end is the (k-1-need) enumerated.
    ...
    public:
        ...
        [[nodiscard]] std::size_t select0(std::size_t r) const noexcept {
            ...
            for (std::size_t b = lo * SampleRate;; ++b) {
                const std::size_t k = derived().bt(b);
                // blen is BlockSize except for the vector final (partial) block.
    ...
    protected:
        ...
        // Offset-bit position of block b offset, walking from its sample group.
...
namespace cds {
    ...
    private:
        ...
        void build(const std::uint64_t* data, std::size_t size) {
            ...
            // geometrically, so without this the arrays keep up to ~2x,
            ...
            // Pass 2: encode block types, sampling every SampleRate-th block.
