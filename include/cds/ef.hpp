...
namespace cds::detail {
    ...
    inline constexpr std::uint32_t ef_magic = 0x65666E34u; // "ef" v4, cds/io format

    // Forward word-scan finding the lowest set bit at or after from_bit, for the
    // iterator's resume-from-last-high-bit decode (see ef::iterator).
    //
    ...
    // real 1-bit (see ef::iterator::operator++).
    ...
    public:
        ...
        // iterator: sequential decode without a fresh select1 per step. Only the
        ...
        public:
            ...
            iterator& operator++() noexcept {
                ensure_cursor();
                ++m_pos;
                // No (size())-th set bit exists, scanning for one would run off
                // the high-bits data. (Dereferencing end() is UB anyway.)
                if (m_pos < m_ef->size()) {
                    if constexpr (Derived::endian == pack_endian::lsb) {
                        ...
                        // empties. Breaks the per-step memory dependency
                        // ef_find_next_set_bit has.
                        m_word &= (m_word - 1);
        ...
        // Value at logical index i. Precondition: i < size().
        ...
        // select1(i) == the i-th value (ef is a select1 structure over the
        // conceptual bit array whose set bits sit at the stored values).
        ...
        // trailing word. Wider widths and msb use the safe extract().
        ...
        // when built via ef::from_deltas). Precondition: i < size() - 1.
        [[nodiscard]] std::uint64_t diff(std::size_t i) const noexcept {
            return derived()[i + 1] - derived()[i];
        }

        // Number of stored values strictly less than k. O(log n) binary search.
        ...
        [[nodiscard]] return_value ple(std::uint64_t x) const noexcept {
            ...
            if constexpr (Derived::index_zeros) {
                ...
                if constexpr (Derived::endian == pack_endian::lsb) {
                    // scan back one set bit from lf's cursor instead of a fresh select1 via
                    // operator[].
...
namespace cds {
    ...
    public:
        ...
        // has size() == deltas.size() + 1; element 0 is always the
        // implicit leading zero. diff(i) for i in [0, deltas.size())
        // recovers the i-th original delta.
        ...
        // Cached largest value, O(1).
        // Overrides ef_ops::back() (a full select1 + decode).
        [[nodiscard]] std::uint64_t back() const noexcept {
            return m_back;
        }
    ...
    private:
        void build(std::span<const std::uint64_t> values) {
            ...
            // Total high-bits length is known upfront (values is sorted): one
            // bulk pre-zeroed allocation, then only the n 1-bit positions.
            ...
            if (m_low_width > 0) {
                // +1 spare word so decode_at's unaligned 64-bit read never
                // runs off the end (see decode_at); 64 extra elements is at
                // least one Word regardless of width.
            ...
            m_high =
                high_bits_type(std::move(hb)); // steal the builder's buffer; no per-bit push_back

            m_select.emplace(m_high);
            m_back = values[m_size - 1]; // build() is only called with m_size > 0
        }
    ...
    // ef_view: non-owning, zero-copy counterpart of ef.
    template <typename Word = std::uint64_t, pack_endian Endian = pack_endian::lsb,
