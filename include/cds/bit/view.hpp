...
namespace cds {
    ...
    public:
        ...
        using impl::impl;
        // NOTE: no `using impl::clear;` — a view (packed_view) has no
        // container clear(); only the bit clear(index) from
        // bit_mutation_ops applies here.
        using bit_mutation_ops<bit_view<Word, Endian>>::clear;

        // Adopt an already-built impl base (load() returns the impl).
