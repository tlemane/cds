...
namespace cds::detail {
    ...
    rank9_rank1(const Word* data, const rank9_superblock* superblocks, std::size_t i) noexcept {
        ...
        // For branchless read (see rank9_build): w == 0 shifts onto the zero bit.
    ...
    // Fused rank1(x) + bit at x. rank1 already reads the word holding bit
    // i for its partial popcount, so the bit comes free.
    // Precondition: i < size (the word is always dereferenced, unlike rank1 at a word boundary)
    template <typename Word, pack_endian Endian>
    ...
    inline void rank9_build(const Word* data, std::size_t size,
        ...
        out[nsb].absolute = absolute; // sentinel = total popcount
    }
...
namespace cds {
    ...
    //
    // Precondition: the source words must outlive this rank9 and must not be
    ...
    public:
        ...
        // rank1(i) and the bit at i from one word load.
        // Precondition: i < size.
    ...
    // rank9_view: non-owning. Binds to a pre-built index (e.g. mmap, or
    // another rank9 superblocks) with no build pass and no copy. The caller
    // keeps both the index and the source words valid for the view lifetime.
    template <bit_source Source> class rank9_view {
    public:
        using word_type = typename bit_source_traits<Source>::word_type;
        static constexpr pack_endian endian = bit_source_traits<Source>::endian;
