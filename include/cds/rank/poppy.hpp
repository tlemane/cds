...
namespace cds::detail {
    ...
    // idx in [0, 2]; block 3's count is derived from the sum of these three.
    ...
    inline void poppy_build(const Word* data, std::size_t size, std::uint64_t upper_block_bits,
        ...
        l0.assign(num_upper_blocks + 1, 0); // +1 sentinel: total popcount
        // +1 sentinel lower block: read by rank1(size) when size is an exact
        // multiple of poppy_lower_block_bits, keeping the query branch-free
        ...
        for (std::size_t lb = 0; lb < num_lower_blocks; ++lb) {
            ...
            // looped in case upper_block_bits is smaller than a lower block
    ...
    // x for its partial popcount, so the bit comes free. Precondition: x < size
    // (the word is always dereferenced, unlike rank1 at a word boundary).
    template <typename Word, pack_endian Endian>
    ...
    inline constexpr std::uint32_t poppy_magic = 0x70707972u;
