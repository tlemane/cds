...
namespace cds::detail {
    ...
    // Precondition: r < arr.num_positions.
    // Arrays is darray_arrays (owning) or darray_arrays_view
    template <typename Word, pack_endian Endian, bool Ones, std::size_t BlockSize,
    ...
    [[nodiscard]] inline std::size_t darray_select(const Word* data, const Arrays& arr,
        ...
        const Word all_ones = static_cast<Word>(~Word{0}); // ~Word{0} promotes to a signed int
        if constexpr (Endian == pack_endian::lsb)
            word = static_cast<Word>(word & static_cast<Word>(all_ones << bit_in_word));
        else
            word = static_cast<Word>(word & static_cast<Word>(all_ones >> bit_in_word));

        std::size_t remaining = need; // need >= 1
        while (true) {
            std::size_t popcnt;
    ...
    inline constexpr std::uint32_t darray_magic = io::cds_magic(io::format_id::darray);

    // Bytes of zero padding needed after a side subblocks
    ...
    [[nodiscard]] inline bool darray_write_side(Sink& sink, const darray_arrays& a) noexcept {
        ...
        // subblocks (uint16), so an odd subblock count cannot misalign
        // it (which would be UB for the zero-copy view)
        if (bi > 0 &&
    ...
    // Advances the reader past one side section for a view that doesn't want this side.
    template <typename ByteSource>
...
namespace cds {
    ...
    // select0 per Target.
    //
    // Reference-holding: dense blocks scan the source raw words at query
    ...
    public:
        ...
        // darray format always writes both sides (even empty ones) regardless
        // of Target, so load() reads both sections
        template <typename ByteSource>
