...
namespace cds {
    ...
    // source. This adapter bridges that for any rank/select pair
    //
    // The select holds pointers into the source words and spans into the
    // rank heap. So the source must outlive this, and copies must rebuild the
    // select (a copied rank has a fresh heap, the original select can't
    // reference it). Moves are fine
