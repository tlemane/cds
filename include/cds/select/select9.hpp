...
namespace cds::detail {
    ...
    // Hint sampling interval, in target-value units. Every hint_spacing
    // matching bits records which rank9 superblock that rank falls into, so a
    // query only binary-searches the superblocks between two consecutive hints.
    ...
    struct select9_header {
        ...
        std::uint64_t nhints;  // 0 if the select1 side wasn't built
        std::uint64_t nhints0; // 0 if the select0 side wasn't built
    };
    static_assert(io::mmap_aligned_header<select9_header>);

    inline constexpr std::uint32_t select9_magic = 0x73656C39u;
...
namespace cds {
    ...
    //
    // Precondition: the rank9 (and the word storage it was built over) must
    ...
    public:
        ...
        // Position of the r-th set bit (0-indexed). Precondition: r < number
        // of set bits.
        ...
        // Position of the r-th unset bit (0-indexed).
    ...
    // select9_view: non-owning counterpart, same Target gating. Specialized
