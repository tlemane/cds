...
namespace cds::detail {
    ...
    // Needed because a partial last block's padding bits would otherwise be
    ...
    // (Ones: one-positions, else zero-positions).
    ...
    // Precondition: r < total count of the target side (ones if
    // Ones=true, zeros if Ones=false).
    template <typename Word, pack_endian Endian, bool Ones>
    ...
    poppy_select(const Word* data, std::size_t size, const std::uint64_t* l0,
        ...
        auto count_before_upper = [&](std::size_t u) noexcept -> std::uint64_t {
            ...
            if constexpr (Ones)
                ...
                return static_cast<std::uint64_t>(u) * upper_block_bits -
                       ones; // cumulative-before: always safe, no clamp needed
        };

        // 1. binary search for the upper block
        ...
        auto count_before_lower_in_upper = [&](std::size_t lb_local,
            ...
            if constexpr (Ones)
                ...
                return static_cast<std::uint64_t>(lb_local) * poppy_lower_block_bits -
                       ones; // cumulative-before: always safe
        };

        // 3. linear scan forward through lower blocks
        ...
        // 4. L2: find the exact basic block
        ...
        // 5. popcnt through words, then broadword select in the target word
        ...
        for (std::size_t w = 0; w < words_per_bb; ++w) {
            ...
            if (actual < digits) {
                // mask padding of the last word (~word would turn padding into
    ...
    // magic(4)+reserved(4) keeps the uint64 fields 8-byte aligned and the
    // struct a multiple of 8 (io::mmap_aligned_header). The version is written
    // separately via io::write_cds_version, not stored here.
    struct poppy_select_header {
        std::uint32_t magic;
        std::uint32_t reserved{0};
        std::uint64_t sample_rate;
        std::uint64_t samples_n; // 0 if the ones side wasn't built
        std::uint64_t offsets_n;
        std::uint64_t samples0_n; // 0 if the zeros side wasn't built
        std::uint64_t offsets0_n;
    };
    static_assert(io::mmap_aligned_header<poppy_select_header>);

    inline constexpr std::uint32_t poppy_select_magic = 0x63737079u; // "cspy"-ish

    // Zero padding after a side's uint32 samples so the uint64 offsets that
    // follow stay 8-byte aligned (samples*4 is only a multiple of 4; an odd
    // count would misalign the offsets and break a zero-copy view).
...
namespace cds {
    ...
    // Target. select0 is an extension beyond the paper (see poppy_actual_bits
    // for the partial-last-block subtlety).
    //
    ...
    public:
        ...
        // Non-owning view over this index, the backing rank_poppy index, and
        // the same source words. Valid only while this select_poppy (and its
        // rank_poppy's source) stay alive.
        ...
        // Bound to an existing rank_poppy. A sample_rate mismatch or a saved
        // file missing a requested side gives type_mismatch.
    ...
    // select_poppy_view: non-owning counterpart of select_poppy. Binds to a
    // pre-built rank_poppy index and pre-built sample arrays (e.g. mmap'd) with
    // no build pass and no copy. The caller keeps the rank index, the samples,
    // and the source words valid for the view's lifetime.
    template <bit_source Source, select_target Target = select_target::ones,
              std::uint64_t UpperBlockBits = (std::uint64_t{1} << 32)>
    class select_poppy_view {
    public:
        ...
        // Zero-copy, bound to a rank_poppy_view: maps each side's uint32 samples
        // and uint64 offsets in place (the samples are padded to keep the
        // offsets 8-byte aligned). Reads only the wanted side(s), skips others.
        template <typename ByteSource>
